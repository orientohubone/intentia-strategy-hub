    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] text-slate-500 uppercase tracking-wider">Serviços</span>
          </div>
          <p className="text-lg font-bold text-white">{operationalCount}/{services.length}</p>
          <p className="text-[10px] text-slate-600">operacionais</p>
        </div>
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-1">
            <div className={`w-2 h-2 rounded-full ${activeIncidents > 0 ? "bg-red-500" : "bg-green-500"}`} />
            <span className="text-[11px] text-slate-500 uppercase tracking-wider">Incidentes</span>
          </div>
          <p className="text-lg font-bold text-white">{activeIncidents}</p>
          <p className="text-[10px] text-slate-600">ativos</p>
        </div>
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-1">
            <div className={`w-2 h-2 rounded-full ${scheduledMaintenances > 0 ? "bg-blue-500" : "bg-green-500"}`} />
            <span className="text-[11px] text-slate-500 uppercase tracking-wider">Manutenções</span>
          </div>
          <p className="text-lg font-bold text-white">{scheduledMaintenances}</p>
          <p className="text-[10px] text-slate-600">agendadas</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 bg-slate-800/40 rounded-lg p-0.5">
        {[
          ...
          >
            {tab.label} {tab.count !== null && <span className="text-slate-600 ml-1">({tab.count})</span>}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-2">
          ...
          {showNewService && (
            <div className="bg-slate-900/80 border border-slate-700 rounded-xl p-4 space-y-3">
              <p className="text-xs font-medium text-white">Novo Serviço</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Input
                  placeholder="Nome do serviço"
                  value={newService.name}
                  onChange={(e) => setNewService({ ...newService, name: e.target.value })}
                  className="h-8 text-xs bg-slate-800/60 border-slate-700 text-white placeholder:text-slate-600"
                />
                <Input
                ...
                <Select value={newService.icon} onValueChange={(v) => setNewService({ ...newService, icon: v })}>
                  <SelectTrigger className="h-8 text-xs bg-slate-800/60 border-slate-700 text-white">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="ghost" className="h-7 text-[11px] text-slate-400" onClick={() => setShowNewService(false)}>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl overflow-hidden">
            {services.map((service, idx) => {
              ...
              return (
                <div key={service.id} className={`${idx > 0 ? "border-t border-slate-800/50" : ""}`}>
                  <div className="flex items-center gap-3 px-4 py-3 hover:bg-slate-800/20 transition-colors">
                    ...
                    <div className="w-8 h-8 rounded-lg bg-slate-800/60 flex items-center justify-center flex-shrink-0">
                      <IconComp className="h-4 w-4 text-slate-300" />
                    </div>
                      ...
                      {isEditing ? (
                        <div className="flex gap-2">
                          <Input
                            ...
                            className="h-7 text-xs bg-slate-800/60 border-slate-700 text-white"
                          />
                          <Input
                        ...
                        <>
                          ...
                          <p className="text-[10px] text-slate-600 truncate">{service.description}</p>
                        </>
                      )}
                    </div>

                    {/* Category */}
                    <Badge className="text-[9px] bg-slate-800/60 text-slate-400 border-slate-700 hidden sm:inline-flex">
                      {service.category}
                    </Badge>
                    ...
                    >
                      <SelectTrigger className="w-[120px] h-7 text-[10px] bg-slate-800/40 border-slate-700/50 text-slate-400 flex-shrink-0">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {isEditing ? (
                        <>
                          ...
                          <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-slate-500 hover:text-slate-300" onClick={() => setEditingService(null)}>
                            <X className="h-3 w-3" />
                          </Button>
                        </>
                      ) : (
                        <>
                          <Button
                            size="sm" variant="ghost" className="h-6 w-6 p-0 text-slate-600 hover:text-slate-300"
                          ...
                          <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-slate-600 hover:text-red-400" onClick={() => deleteService(service.id)}>
      ...
      {subTab === "incidents" && (
        <div className="space-y-2">
          ...
          {/* New incident form */}
          {showNewIncident && (
            <div className="bg-slate-900/80 border border-red-500/20 rounded-xl p-4 space-y-3">
              <p className="text-xs font-medium text-white">Novo Incidente</p>
              <Input
              ...
              {/* Affected services */}
              <div>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Serviços Afetados</p>
                    ...
                    return (
                      <button
                        ...
                        className={`px-2 py-1 rounded text-[10px] font-medium transition-colors ${
                          selected ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-slate-800/60 text-slate-500 border border-slate-700/50 hover:text-slate-300"
                        }`}
                      >
                        {s.name}
                      </button>
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="ghost" className="h-7 text-[11px] text-slate-400" onClick={() => setShowNewIncident(false)}>
                  <X className="h-3 w-3 mr-1" /> Cancelar
                </Button>
              </div>
            </div>
          ...
          {incidents.length === 0 ? (
            ...
            <div className="space-y-2">
              {incidents.map((incident) => {
                ...
                return (
                  <div key={incident.id} className="bg-slate-900/60 border border-slate-800 rounded-xl overflow-hidden">
                    <button
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <Badge className={`text-[9px] ${statusCfg.color} bg-slate-800/60 border-slate-700`}>
                          {statusCfg.label}
                        </Badge>
                        ...
                        <div className="bg-slate-800/30 rounded-lg p-3 space-y-2">
                          ...
                          <div className="flex gap-2">
                            <Select value={newUpdateStatus} onValueChange={setNewUpdateStatus}>
                              <SelectTrigger className="w-[120px] h-7 text-[10px] bg-slate-800/60 border-slate-700 text-white">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                {Object.entries(INCIDENT_STATUS_CONFIG).map(([key, cfg]) => (
                            ...
                            <Input
                              ...
                              className="h-7 text-[10px] bg-slate-800/60 border-slate-700 text-white placeholder:text-slate-600 flex-1"
                            />
                            <Button
                              size="sm"
                            ...
                            {incidentUpdates.map((update) => {
                              ...
                              return (
                                <div key={update.id} className="relative">
                                  <div className="absolute -left-[17px] top-1 h-2 w-2 rounded-full bg-slate-700 border border-slate-600" />
                                  <div className="flex items-center gap-2 mb-0.5">
                                    <span className={`text-[10px] font-medium ${uCfg.color}`}>{uCfg.label}</span>
                                    <span className="text-[10px] text-slate-600">{formatDateTime(update.created_at)}</span>
                                  </div>
                                  <p className="text-[11px] text-slate-400">{update.message}</p>
                                </div>
                              );
                            })}
                          </div>
      ...
      {subTab === "maintenances" && (
        <div className="space-y-2">
          ...
          {showNewMaintenance && (
            <div className="bg-slate-900/80 border border-blue-500/20 rounded-xl p-4 space-y-3">
              <p className="text-xs font-medium text-white">Nova Manutenção Programada</p>
              <Input
              ...
              <Textarea
                ...
                className="text-xs bg-slate-800/60 border-slate-700 text-white placeholder:text-slate-600 min-h-[60px]"
              />
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-[10px] text-slate-500 mb-1">Início Programado</p>
                  <Input
                    type="datetime-local"
                    value={newMaintenance.scheduled_start}
                    onChange={(e) => setNewMaintenance({ ...newMaintenance, scheduled_start: e.target.value })}
                    className="h-8 text-xs bg-slate-800/60 border-slate-700 text-white"
                  />
                </div>
                <div>
                  <p className="text-[10px] text-slate-500 mb-1">Fim Programado</p>
                  <Input
                    type="datetime-local"
                  ...
                  {services.map((s) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-2 py-1 rounded text-[10px] font-medium transition-colors ${
                          selected ? "bg-blue-500/20 text-blue-400 border border-blue-500/30" : "bg-slate-800/60 text-slate-500 border border-slate-700/50 hover:text-slate-300"
                        }`}
                      >
                        {s.name}
                      </button>
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="ghost" className="h-7 text-[11px] text-slate-400" onClick={() => setShowNewMaintenance(false)}>
                  <X className="h-3 w-3 mr-1" /> Cancelar
                </Button>
              </div>
            </div>
          ...
          {maintenances.length === 0 ? (
            ...
            <div className="space-y-2">
              {maintenances.map((m) => {
                ...
                return (
                  <div key={m.id} className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-slate-600 hover:text-red-400 flex-shrink-0" onClick={() => deleteMaintenance(m.id)}>
                        <Trash2 className="h-3 w-3" />
                      </Button>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-[10px] text-slate-500 mb-3">
                      ...
                      {m.affected_services.length > 0 && (
                        <span className="text-slate-600">· {m.affected_services.map(getServiceName).join(", ")}</span>
                      )}
                    </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gradient-to-r from-primary/10 to-orange-500/5 border border-primary/20 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-sm font-bold text-white">Guia de Gerenciamento da Status Page</h3>
            </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-white uppercase tracking-wider">Gerenciando Serviços</h4>
            </div>
            <div className="space-y-2 text-xs text-slate-400">
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">1.</span>
                <p>Na aba <strong className="text-slate-300">Serviços</strong>, clique em <strong className="text-slate-300">+ Novo Serviço</strong> para adicionar um serviço que será exibido na página de status.</p>
              </div>
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">2.</span>
                <p>Preencha <strong className="text-slate-300">nome</strong>, <strong className="text-slate-300">categoria</strong> (Core, Integrações, Analytics, etc.), <strong className="text-slate-300">ícone</strong> e <strong className="text-slate-300">ordem de exibição</strong>.</p>
              </div>
              <div className="flex gap-2">
              ...
              <div className="bg-slate-800/50 rounded-lg p-2.5 mt-1">
                <div className="flex items-start gap-1.5">
                  <Info className="h-3.5 w-3.5 text-blue-400 mt-0.5 shrink-0" />
                  <p className="text-slate-500">O status geral da página é determinado automaticamente pelo pior status entre todos os serviços visíveis.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-white uppercase tracking-wider">Gerenciando Incidentes</h4>
            </div>
            <div className="space-y-2 text-xs text-slate-400">
              <p><strong className="text-slate-300">Quando criar um incidente:</strong> Sempre que houver um problema real que afete usuários (lentidão, erros, indisponibilidade).</p>
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">1.</span>
                <p>Na aba <strong className="text-slate-300">Incidentes</strong>, clique em <strong className="text-slate-300">+ Novo Incidente</strong>.</p>
              </div>
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">2.</span>
                <p>Defina o <strong className="text-slate-300">título</strong> (ex: "Lentidão na API de Análise"), a <strong className="text-slate-300">severidade</strong> e selecione os <strong className="text-slate-300">serviços afetados</strong>.</p>
              </div>
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">3.</span>
                <p>Adicione <strong className="text-slate-300">atualizações na timeline</strong> conforme o problema evolui. Clique no incidente para expandir e usar o campo de atualização.</p>
              </div>
              <div className="flex gap-2">
              ...
              <p className="text-slate-500 mt-2 font-medium">Severidades:</p>
              ...
              <p className="text-slate-500 mt-2 font-medium">Fluxo recomendado de status:</p>
              ...
              <div className="bg-slate-800/50 rounded-lg p-2.5 mt-1">
                <div className="flex items-start gap-1.5">
                  <Info className="h-3.5 w-3.5 text-blue-400 mt-0.5 shrink-0" />
                  <p className="text-slate-500">Incidentes <strong className="text-slate-400">critical</strong> e <strong className="text-slate-400">major</strong> disparam notificação por email automaticamente (se configurado).</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-white uppercase tracking-wider">Gerenciando Manutenções</h4>
            </div>
            <div className="space-y-2 text-xs text-slate-400">
              <p><strong className="text-slate-300">Quando agendar:</strong> Sempre que houver uma janela de manutenção planejada que possa afetar a disponibilidade.</p>
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">1.</span>
                <p>Na aba <strong className="text-slate-300">Manutenções</strong>, clique em <strong className="text-slate-300">+ Nova Manutenção</strong>.</p>
              </div>
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">2.</span>
                <p>Defina <strong className="text-slate-300">título</strong>, <strong className="text-slate-300">descrição</strong>, <strong className="text-slate-300">data/hora de início e fim</strong>, e selecione os <strong className="text-slate-300">serviços afetados</strong>.</p>
              </div>
              <div className="flex gap-2">
                <span className="text-primary font-bold shrink-0">3.</span>
              ...
              <div className="bg-slate-800/50 rounded-lg p-2.5 mt-1">
                <div className="flex items-start gap-1.5">
                  <Info className="h-3.5 w-3.5 text-blue-400 mt-0.5 shrink-0" />
                  <p className="text-slate-500">Manutenções agendadas e em andamento aparecem automaticamente na seção "Manutenções Programadas" da página <code className="text-primary">/status</code>.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-white uppercase tracking-wider">Como Funciona o Sistema</h4>
            </div>
            <div className="space-y-3 text-xs text-slate-400">
              <div className="space-y-1.5">
                <p className="text-slate-300 font-medium">Fluxo de um incidente:</p>
                <div className="bg-slate-800/50 rounded-lg p-3 space-y-1 font-mono text-[10px] text-slate-500">
              ...
              <div className="space-y-1.5">
                <p className="text-slate-300 font-medium">Automações ativas:</p>
                <div className="space-y-1">
                  <div className="flex items-start gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5 shrink-0" />
                    <p><strong className="text-slate-300">collect-uptime</strong> — Roda 1x/dia via cron. Calcula e registra o uptime de cada serviço baseado no status e incidentes do dia.</p>
                  </div>
                  <div className="flex items-start gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5 shrink-0" />
                    <p><strong className="text-slate-300">notify-incident</strong> — Envia email quando incidente critical/major é criado. Sem email configurado, registra no audit log.</p>
                  </div>
                  <div className="flex items-start gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5 shrink-0" />
                    <p><strong className="text-slate-300">status-rss</strong> — Feed RSS público com incidentes dos últimos 90 dias. Link aparece na página /status.</p>
                  </div>
                  <div className="flex items-start gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5 shrink-0" />
                    <p><strong className="text-slate-300">status-webhook</strong> — Recebe dados de UptimeRobot/Checkly e atualiza status + latência dos serviços.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex items-center justify-between bg-slate-800/50 rounded-lg px-3 py-2">
                <span className="text-slate-400">Página de Status (pública)</span>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-white uppercase tracking-wider">Boas Práticas</h4>
            </div>
            <div className="space-y-1.5 text-xs text-slate-400">
              <div className="flex items-start gap-2">
                <span className="text-amber-500 mt-0.5">•</span>
                <p>Crie incidentes <strong className="text-slate-300">rapidamente</strong> — transparência gera confiança. Melhor informar cedo do que tarde.</p>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-amber-500 mt-0.5">•</span>
                <p>Adicione <strong className="text-slate-300">atualizações frequentes</strong> na timeline — mesmo que seja "Ainda investigando". Silêncio gera ansiedade.</p>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-amber-500 mt-0.5">•</span>
                <p>Use <strong className="text-slate-300">severidade correta</strong> — não marque tudo como "Crítico". Reserve para indisponibilidade total.</p>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-amber-500 mt-0.5">•</span>
                <p>Agende manutenções com <strong className="text-slate-300">antecedência</strong> — idealmente 48h antes, para que os usuários se preparem.</p>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-amber-500 mt-0.5">•</span>
                <p>Após resolver, escreva um <strong className="text-slate-300">post-mortem breve</strong> na última atualização do incidente explicando causa e ação corretiva.</p>
              </div>
            </div>
          </div>
        </div>
